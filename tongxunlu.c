...
void Contactbc(struct ps* s)
{
	int i = 0;
	FILE* ert = fopen("text.txt", "w");
	if (ert == NULL)
	{
		printf("Contacttc:%s\n",strerror(ert));
		return;
	...
	for (i = 0; i < s->size; i++)
	{
		 fwrite(&(s->renshu[i]), sizeof(sd), 1, ert);
	}
	fclose(ert);
	ert = NULL;
}
void Contactcs(struct ps* s)
{
	FILE* ert = fopen("text.txt", "r");
	sd tmp={ 0 };
	if (ert == NULL)
	{
		printf("Contactcs:%s\n", strerror(ert));
	...
	while (fread(&tmp, sizeof(sd), 1, ert))
	{
		kuoren(s);
		s->renshu[s->size] = tmp;
		s->size++;
